...
dashboard.displayMember = function(member, circleUi) {
  ...
  const name = memberUi.querySelector('.member-name').innerText
  const location = member.location;
  const latlonSp = latlonUi.innerText.replaceAll(" ", '').split(",");
  if(isSwitched < 6 && latlonSp[0]+","+latlonSp[1] != ",undefined"){
  ...
  console.log(batteryIconUi)
  var htmlToElements = `<div style="width: 100%"><h1>${name}:</h1><br><h2 style="font-size: 25px;">🔋 ${batteryTextUi.innerText}%</h2><br><iframe width="100%" height="600" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" id="${name.replaceAll(" ", "-")}" src="https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=${latlonSp[0]+","+latlonSp[1]}+(lol)&amp;t=&amp;z=14&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"><a href="https://www.maps.ie/distance-area-calculator.html">measure distance on map</a></iframe></div><br>`.toDOM();
  document.body.appendChild(htmlToElements);
  var oldCord = latlonSp[0]+","+latlonSp[1];
  console.log("got location for "+name+"\n"+oldCord)
  isSwitched++;
}else if(latlonSp[0]+","+latlonSp[1] != ",undefined" && !latlonSp[0]+","+latlonSp[1] == oldCord){
    var iframe = document.getElementById(name.replaceAll(" ", "-"))
    iframe.src = `https://maps.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q=${latlonSp[0]+","+latlonSp[1]}+(lol)&amp;t=&amp;z=14&amp;ie=UTF8&amp;iwloc=B&amp;output=embed`;  
    console.log("Updated location for "+name+"\n"+latlonSp[0]+","+latlonSp[1])
    var oldCord = latlonSp[0]+","+latlonSp[1]
}
  if (location) {
    if (location.battery) {
